...
void display(void)
{
    ...
    glPushMatrix();
        ...
        glPopMatrix();
        if(turn)    glTranslatef(-list[lenght-1].mid_x, view_y-2.3f, place);
        else        glTranslatef(-place, view_y-2.3f, list[lenght-1].mid_z);
        glMaterialfv(GL_FRONT, GL_AMBIENT, no_mat);
        glMaterialfv(GL_FRONT, GL_DIFFUSE, mat_diffuse);
        glMaterialfv(GL_FRONT, GL_SPECULAR, mat_specular);
        glMaterialfv(GL_FRONT, GL_SHININESS, high_shininess);
        glMaterialfv(GL_FRONT, GL_EMISSION, no_mat);
        glScalef(list[lenght-1].x, 0.7f, list[lenght-1].z);
        glutSolidCube(1.0);
    glPopMatrix();
...
bool addCube(void){
    ...
    if(fabsf(err)>0.1f){
        list[lenght].f=true;
        list[lenght].h=view_y-2.3f;
        list[lenght+1].f=false;
        list[lenght+1].h=view_y-2.3f;
        if(turn){
...
void drawList(void){
    for(int i=0;i<lenght;i++){
        ...
        glMaterialfv(GL_FRONT, GL_SHININESS, high_shininess);
        glMaterialfv(GL_FRONT, GL_EMISSION, no_mat);
        glScalef(list[i].x, 0.7f, list[i].z);
        glutSolidCube(1.0);
        glPopMatrix();
    }
}

...
void display(void)
{
	...
	if (isEnd)   endGame();
		...
		char* start = "PRESS SPACE TO PLAY THIS GAME"; int n = (int)strlen(start);
		gluLookAt(-5.0, view_y, 5.0, 0.0, view_y - 3.0f, 0.0, 0.0, 1.0, 0.0);
		glDisable(GL_LIGHTING);
		glColor3f(0.4f, 0.0f, 0.9f);
		glRasterPos3f(-1.2f, view_y -0.2f, 0.0f);
		for (int i = 0; i < n; i++)
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case ' ':
		...
		else if (!startGame){ startGame = !startGame;  glutPostRedisplay(); }
		else            CubeCut();
		break;
	}
}
...
void init(void)
{
	glClearColor(0.7f, 1.0f, 0.9f, 0.5f);
...
void endGame(void){
	...
	glDisable(GL_LIGHTING);
	glColor3f(0.4f, 0.0f, 0.9f);
	glRasterPos3f(-0.4f, view_y, 0.0f);
	for (int i = 0; i <11; i++)
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, *(over + i));
	glRasterPos3f(-0.3f, view_y - 0.5f, 0.0f);
	for (int i = 0; i <11; i++)
